...
* 本地环境：
  ...
  5. **练习rustlings**。在项目根目录执行`rustlings watch`依次查看完成情况，并依次完成对应的练习。 执行`rustlings run 练习名称`去运行对应练习，也可以使用`rustlings hint 练习名称`查看题解。使用vscode等编辑器，修改项目目录下的`exercises`文件夹中的文件完成练习. 
